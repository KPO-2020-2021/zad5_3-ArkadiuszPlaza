...
/*!
 * \brief Klasa pomocnicza, pozwala modelować Vector dwuwymiarowy
 *
 * Modeluje Vector dwuwymiarowy wraz z podstawymi operacjami.
 */
 class Vector2f {
    public:
     /*!
      * \brief Współrzędna x-owa Vectora
      *
      * Zawiera wartość współrzędnej x-owej Vectora.
      */
      float  x;
     /*!
      * \brief Współrzędna y-owa Vectora
      *
      * Zawiera wartość współrzędnej y-owej Vectora.
      */   
      float  y;

     /*!
      * \brief Inicjalizuje współrzędne Vectora wartościami 0
      *
      * Inicjalizuje współrzędne Vectora wartościami 0.
      */
      Vector2f(): x(0), y(0) {}
     /*!
      * \brief Inicjalizuje współrzędne Vectora wartościami parametrów.
      *
      * Inicjalizuje współrzędne Vectora wartościami parametrów.
     ...
     /*!
      * \brief Dodanie i podstawienie do współrzędnych, wartości innego Vectora
      *
      * Dodanie i podstawienie do współrzędnych, wartości innego Vectora.
      * \param[in] Skl - Vector, które współrzędne mają zostać dodane, do 
      *                  Vectora \p *this.
      * \return referencję do Vectora \p *this, dla którego wywołana zostaje ta metoda. 
      */
      Vector2f& operator += (const Vector2f& Skl) 
                       { x += Skl.x;  y += Skl.y;  return *this; }

     /*!
      * \brief Dodanie do współrzędnych, wartości innego Vectora
      *
      * Dodanie o współrzędnych, wartości innego Vectora.
      * \param[in] Skl2 - drugi argument operacji dodawania, którego współrzędne mają zostać dodane,
      *                  do Vectora \p *this.
      * \return wynik operacji dodawania, który jest sumą Vectora \p *this i \e Skl2.
      */   
      Vector2f  operator + (Vector2f Skl2) const
                       { return Skl2 += *this; }

     /*!
      * \brief Mnożenie przez liczbę współrzędnych Vectora i ich podstawienie
      *
      * Mnożenie przez liczbę współrzędnych Vectora i ich podstawienie.
      * \param[in] Arg - liczba, przez którą mają być pomnożone współrzędne Vectora, a
      *                  następnie podstawione w miejsce starych współrzędnych.
      * \return referencję do Vectora \p *this, dla którego wywołana zostaje ta metoda. 
      */
      Vector2f& operator *= (float Arg) 
                       { x *= Arg;  y *= Arg;  return *this; }

     /*!
      * \brief Mnożenie przez liczbę współrzędnych Vectora
      *
      * Mnożenie przez liczbę współrzędnych Vectora.
      * \param[in] Arg - liczba, przez którą mają być pomnożone współrzędne Vectora.
      * \return Vectora, który zawiera wynik wykonanej operacji.
      */   
      Vector2f  operator * (float Arg) const 
                       { return Vector2f(*this) *= Arg; }

     /*!
      * \brief Modyfikuje współrzędne Vectora
      *
      * Modyfikuje współrzędne Vectora.
      * \param[in] x - nowa wartość współrzędnej x-owej,
      * \param[in] y - nowa wartość współrzędnej y-owej.
      */
      void Zmien(float x, float y) {  Vector2f::x = x;  Vector2f::y = y; }
 };
 ...
 */
 istream& operator >> (istream& StrmWe, Vector2f &Wspolrz)
 {
 ...
 */
 class InfoPunktyDoRysowania {
    ...
    static  Vector2f   _PrzesEtykiety_WzgRozmPunktu;
      ...
      */
    Vector2f  WyznaczWspolrzEtykiety() const
     {
   ...
   public:
     ...
     */
     InfoPunktyDoRysowania( const Vector2f           &Wspolrz, 
     ...
     */
     const Vector2f&  WezWspolrzEtykiety() const 
     ...
     */
    const Vector2f& WezWspolrz() const { return _Wspolrz; }
    /*!
     ...
     */
    Vector2f& OtworzWspolrz() { return _Wspolrz; }
    /*!
...
Vector2f InfoPunktyDoRysowania::_PrzesEtykiety_WzgWspoPunktu(0.5,0.5);
Vector2f InfoPunktyDoRysowania::_PrzesEtykiety_WzgRozmPunktu(0.26,0.26);
